...
Once you've opened a file that's a real match, use `links` (`kind: "forward"` or `"backlinks"`, with `document` set to that file's path) to see what it's directly connected to -- a concept's neighbors in the knowledge base are often exactly the other pieces a full answer needs, and following an explicit link is more reliable than guessing at new search terms. For a broad or unfamiliar question, where nothing in your first search stood out as the obvious starting point, try `links` with `kind: "hubs"` first -- it surfaces the most-connected pages in the knowledge base, which tend to be good entry points into a topic.
