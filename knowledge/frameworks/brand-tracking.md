...
with resulting uplift using return-on-investment measures.
...
activity is intended to produce a more immediate response.
...
short-term activation and immediate uplift.
...
universal timing rule is specified.
...
are contextual benchmarks, not universal targets.
...
the combined effect of a multiple-channel media mix.
...
every brand-tracking application.
...
- Do not use overall company sales or profit as a clean proxy for communication
  performance.
- Do not turn an example benchmark into a universal target.
...
- Do not use the MORTN scale as a consumer brand-tracking questionnaire. Its
  ...
  orientation using a ten-item, five-point instrument.
...
assessed through brand tracking.
